...
load(
    "@cuda_redist_json//:build_defs.bzl",
    "get_cuda_distributions",
    "get_cudnn_distributions",
)
...
def _cuda_http_archive_impl(repository_ctx):
    ...
    if cuda_or_cudnn_version:
        ...
        if arch_key in repository_ctx.attr.relative_url_dict.keys():
            (url, sha256) = repository_ctx.attr.relative_url_dict[arch_key]
...
def _get_cuda_archive(
        ...
        dist_dict,
        dist_name,
        cuda_dist_path_prefix = _CUDA_DIST_PATH_PREFIX,
        cudnn_dist_path_prefix = _CUDNN_DIST_PATH_PREFIX,
        dist_extension = ".tar.xz",
        build_file = None,
        build_template = None,
...
def _cuda_distributions(
        cuda_nccl_wheel_dict,
    ...
    cudnn_distributions = get_cudnn_distributions() | cudnn_dist_dict
    ...
    _get_cuda_archive(
        ...
        is_cudnn_dist = True,
        cudnn_dist_path_prefix = cudnn_dist_path_prefix,
        dist_extension = dist_extension,
    )

    cuda_distributions = get_cuda_distributions() | cuda_dist_dict
    _get_cuda_archive(
        repo_name = "cuda_cccl",
...
def hermetic_cuda_init_redist_repositories(
        cuda_nccl_wheel_dict = _CUDA_NCCL_WHEEL_DICT,
        cuda_dist_path_prefix = _CUDA_DIST_PATH_PREFIX,
        cudnn_dist_path_prefix = _CUDNN_DIST_PATH_PREFIX,
        dist_extension = ".tar.xz",
        cuda_dist_dict = {},
        cudnn_dist_dict = {}):
    _cuda_distributions(
        cuda_nccl_wheel_dict = cuda_nccl_wheel_dict,
        cuda_dist_path_prefix = cuda_dist_path_prefix,
        cudnn_dist_path_prefix = cudnn_dist_path_prefix,
        dist_extension = dist_extension,
        cuda_dist_dict = cuda_dist_dict,
        cudnn_dist_dict = cudnn_dist_dict,
    )
