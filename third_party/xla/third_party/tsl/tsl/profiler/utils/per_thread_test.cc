...
class ThreadFactory {
 public:
  ...
  ~ThreadFactory() {
    for (auto threads : threads_existing_at_) {
      while (!threads.empty()) {
  ...
  tsl::Thread* Start(int32_t id, ProfilingStage firstUseStage,
                     ProfilingStage exitStage) {
    std::string name = absl::StrCat("thread_", id);
    auto t = Env::Default()->StartThread(ThreadOptions(), name, [=]() {
      ThreadMain(id, firstUseStage, exitStage);
    });
    threads_existing_at_[exitStage].push_back(t);
    return t;
  }

  void StopAllAt(ProfilingStage exitStage) {
    auto& threads = threads_existing_at_[exitStage];
    while (!threads.empty()) {
      auto* t = threads.front();
      threads.pop_front();
      delete t;
 ...
 private:
  std::vector<std::list<tsl::Thread*>> threads_existing_at_;
};
