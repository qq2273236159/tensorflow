...
TEST(HostKernelTest, JitAddition) {
  ...
  const KernelArgsDeviceMemoryArray ka{args, 0};
  TF_ASSERT_OK(
      executor->Launch(stream.get(), ThreadDim(4), BlockDim(1), *add, ka));
...
static void BM_JitKernel(benchmark::State& state) {
  ...
  const KernelArgsDeviceMemoryArray ka{args, /*shared_memory_bytes=*/0};

  for (auto _ : state) {
    benchmark::DoNotOptimize(executor->Launch(
        stream.get(), ThreadDim(input_size), BlockDim(1), *add, ka));
  }
}
