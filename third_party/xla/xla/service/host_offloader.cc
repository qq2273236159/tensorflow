...
// computation.
std::vector<InstructionAndShapeIndex> GetSuccessors(
  ...
  while (!queue.empty()) {
    ...
    if (instruction->opcode() == HloOpcode::kCustomCall &&
      ...
      if (instruction->opcode() == HloOpcode::kAsyncStart) {
        // Set the host memory space of the input of the async-start. The shape
        // of async-start is ((inputs), output, context), hence the two 0s added
        // to the index.
        ShapeIndex tmp_index = instruction_and_shape_index.shape_index;
        tmp_index.push_front(0);
        tmp_index.push_front(0);
        buffers_to_set_to_host_memory.emplace_back(instruction, tmp_index);
...
std::vector<InstructionAndShapeIndex> HostOffloader::GetStartingInstructions(
    HloInstruction* custom_call_instruction) {
  ...
  std::vector<InstructionAndShapeIndex> result;
  std::queue<InstructionAndShapeIndex> queue;
  const auto successors_of_custom_call =
  ...
  while (!queue.empty()) {
    ...
    const auto successors = GetSuccessors(instruction_and_shape);
    for (const InstructionAndShapeIndex& successor : successors) {
      queue.push(successor);
    }
  }
...
absl::Status HostOffloader::ValidateSliceLeadsToMoveToDeviceCustomCall(
  ...
  std::queue<InstructionAndShapeIndex> queue;
  for (const InstructionAndShapeIndex& successor :
        ...
        if (caller->opcode() == HloOpcode::kWhile) {
          ...
          while (!nested_queue.empty()) {
            ...
            SetMemorySpace(
                ...
                kHostMemorySpaceColor);
            const auto successors = GetSuccessors(nested_instruction_and_shape);
            for (const InstructionAndShapeIndex& successor : successors) {
