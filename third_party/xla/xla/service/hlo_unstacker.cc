...
// A struct that holds the required information for unstacking that is fixed
// across different unstacker instastances.
...
struct UnstackerTransformer {
  ...
  bool HandleInstruction(const HloInstruction* instr, int64_t changed_idx) {
    ...
    for (const auto& [custom_pattern, custom_handler] :
      ...
      // We assume the stacked dimension is always the most major dimension. It
  ...
  absl::flat_hash_map<HloInstruction*, int64_t> oeprand_changes;
};
...
// 1. Hoising the unstacking computation outside the while_instr.
...
// index.
bool UnstackHelper(const UnstackerMetadata& metadata,
  ...
  if (can_unstack) {
    ...
    // If unstacker has not found an unstackable shape, there is not point in
    ...
    for (const auto& [instr, index] : unstacker.oeprand_changes) {
      switch (instr->opcode()) {
...
const HloInstruction* IsDynamicSlicingFusion(const UnstackerMetadata& metadata,
  ...
  if (!Match(instr, match::Fusion(match::Op(), match::Op()))) {
    return nullptr;
  }
    ...
    if (Match(fused_instr, match::DynamicSlice()) &&
      ...
      if (Match(instr->fused_instructions_computation()->root_instruction(),
        ...
        if (bitcast_operand == fused_instr) {
          std::cout << "FARZIN_FOUND IsDynamicSlicingFusion" << std::endl;
          return instr;
        }
      }
    }
...
const HloInstruction* IsNestedDynamicSlicingFusion(
  ...
  for (HloInstruction* inner_fusion_instr :
    ...
    if (Match(inner_fusion_instr, match::DynamicSlice()) &&
        MatchShapeCoveringDynamicIndexInstruction(
            ...
            HloOpcode::kDynamicSlice, while_instr_config)) {
      std::cout << "FARZIN_FOUND IsNestedDynamicSlicingFusion" << std::endl;
...
// corresponding (IsNestedDynamicSlicingFusion) patter to unstack.
...
absl::StatusOr<bool> HloUnstacker::Run(
  ...
  metadata.custom_handlers.push_back(std::make_pair(
      IsNestedDynamicSlicingFusion, UnstackNestedDynamicSlicingFusion));
  ...
  for (HloInstruction* instr :
    ...
    for (int64_t i = 0; i < instr->shape().tuple_shapes_size(); ++i) {
      ...
      if (UnstackHelper(metadata, instr, i)) {
  ...
  if (unstacked) {
    std::cout << "FARZIN_UNSTACKED" << std::endl;
