...
int ActualNumElementsInKernel(int k_row_start, int k_col_start,
  ...
  for (int k_col = 0; k_col < pool.kw; ++k_col) {
    for (int k_row = 0; k_row < pool.kh; ++k_row) {
      ...
      const bool row_in_bottom_bound = target_row < pool.h_in + (2 * pool.ph);
      const bool row_in_top_bound = 0 <= target_row;
      const bool row_in_bound = row_in_top_bound && row_in_bottom_bound;

      const bool col_in_right_bound = target_col < pool.w_in + (2 * pool.pw);
      const bool col_in_left_bound = 0 <= target_col;
      const bool col_in_bound = col_in_left_bound && col_in_right_bound;
...
// Gets a matrix which corrects the overcounting of dividends when casting a
...
DenseFPElementsAttr GetCorrectionMatrix(Builder& builder, CompositeOp op) {
  ...
  // zones will be counted as present elements. Therefor in some windows we
  // will overcount the dividends.
  //    Following from (LEMMA 1) the only windows which contain overcounted
  // dividends are the ones on the outside right and bottom edge. We can iterate
  ...
  // dividends resulting in an equivalant computation.
  {
